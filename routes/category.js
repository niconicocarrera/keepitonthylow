...
router.get('/:recordid/show', function(req, res, next) {
    let query = "SELECT category_id, categoryname, descriptionr FROM category WHERE category_id = " +
    req.params.recordid;
    // execute query
    db.query(query, (err, result) => {
    if (err) {
...
router.post('/', function(req, res, next) {
    ...
    db.query(insertquery,[req.body.categoryname, eq.body.description],(err, result) => {
    if (err) {
    console.log(err);
    res.render('error');
    } else {
